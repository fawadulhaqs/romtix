<template>
    <p class="text-h2 pa-5">Population Group</p>
    <hr class="my-5">
    ...
    <v-list density="compact" class="bg-lightprimary">
        ...
        <v-list-item v-for="list in groups" :key="list.id">
            {{ list.name }}
            <template v-slot:append>
                <label class="switch">
    ...
    export default{
        ...
        methods:{
            update(e,id){
                ...
                var messageObject = {
                    requestType: "get",
                    messageType: `setUserPopulationGroup?groupId=${id}&status=${status}`,
                };
            ...
            getPopulationGroup(){
                var messageObject = {
                    requestType: "get",
                    messageType: `getUserPopulationGroups`,
                };
                this.$store.dispatch("callWebService", messageObject)
                .then((result) => {
                    this.groups = result.populationGroups
                })
                .catch((response) => {
                    console.log(response.data.error);
                })
